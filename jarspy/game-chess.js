...
const jarspy = async (m, { conn, command, args }) => { 
   const key = m.chat; 
...
◦ *${command} create* - Mulai permainan catur 
◦ *${command} join* - Bergabung dalam permainan catur yang sedang menunggu 
◦ *${command} start* - Memulai permainan catur jika ada dua pemain yang sudah bergabung 
◦ *${command} delete* - Menghentikan permainan catur 
◦ *${command} [dari] [ke]* - Melakukan langkah dalam permainan catur 
  
⌕ *Contoh:*
◦ Ketik *${command} create* untuk memulai permainan catur. 
◦ Ketik *${command} join* untuk bergabung dalam permainan catur yang sedang menunggu. 
◦ ◦ *${command} h2 h3* - Melakukan langkah dalam permainan catur 
     `, m); 
   } 
   return conn.reply(m.chat, '⚠️ Perintah tidak valid. Gunakan *"chess help"* untuk melihat bantuan.', m); 
}; 
  
jarspy.help = ['catur']; 
jarspy.tags = ['game']; 
jarspy.command = /^(chess|catur)$/i; 
