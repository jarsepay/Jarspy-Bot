...
import tqto from '../jarspy/jarspy-tqto.js'
import Helper from './helper.js'
import { HelperConnection } from './simple.js'
import script from '../jarspy/jarspy-script.js'
 ...
 *  connectionUpdate?: (update: import('@whiskeysockets/baileys').BaileysEventMap['connection.update']) => any; 
 *  credsUpdate?: () => void 
 * }} EventHandlers
...
/** @type {Map<string, { conn: Socket, store: typeof store }>} */
...
async function start(oldSocket = null, opts = { store, logger, authState }) {
    ...
    await reload(conn, false, opts)
        .then((success) => console.log('- bind handler event -', success))
...
async function reload(conn, restartConnection, opts = { store, authState }) {
    ...
    console.log('Handler', opts.handler)
