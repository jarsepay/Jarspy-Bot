...
const pluginFolder = Helper.__dirname(join(__dirname, '../jarspy'))
...
async function loadPluginFiles(
    ...
    await Promise.all(paths.map(async path => {
        ...
        try {
            ...
            if (module) jarspy[formatedFilename] = module
        } catch (e) {
            opts.logger?.error(e, `error while requiring ${formatedFilename}`)
            delete jarspy[formatedFilename]
        }
    }))
    ...
    return jarspy = sortedPlugins(jarspy)
}
 ...
 * }} 
 */
function sortedPlugins(jarspy) {
    return Object.fromEntries(Object.entries(jarspy).sort(([a], [b]) => a.localeCompare(b)))
}
